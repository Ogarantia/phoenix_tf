...
def plot_exec_time_vs_batch_size(data, use_bias=True):
  ...
  g = sns.FacetGrid(data, row="input_features", col="output_features", hue="engine", sharey=False)
  ...
  plt.savefig(f'{operation}/{operation}_execution_time_vs_batch_size_for_use_bias_{use_bias}.png')

def plot_exec_time_vs_iter_for_specific_batchsize_imgsize(data, batch_size=16, use_bias=True):
  ...
  plt.savefig(f"{operation}/{operation}_Execution_time_per_iteration_for_use_bias_{use_bias}_and_batch_size_of_{batch_size}.png")

def plot_exec_time_vs_iter_for_specific_features(data, input_features=4096, output_features=4096):
  ...
  g = sns.FacetGrid(data, row="batch_size", col="use_bias", hue="engine", sharey=False)
  g.map(sns.lineplot, "iter", "execution time")
  ...
  plt.savefig(f"{operation}/{operation}_Execution_time_per_iteration_for_{input_features}_input_features_and_{output_features}_output_features.png")
