...
def Model(framework, upstride, datatype_int=0, factor=1, input_shape=(3, 32, 32), nclasses=10):
  ...
  x = framework.Conv2D(12 // factor, 3, use_bias=False, activation=tf.nn.relu, **kwargs)(x)
  x = framework.Conv2D(4 // factor, 3, use_bias=True, padding='VALID', dilation_rate=[2, 3], activation=tf.nn.relu)(x)
  x = framework.Conv2D(8 // factor, 3, use_bias=False, padding='SAME', activation=tf.nn.relu)(x)
